...
class PaymentRequestResponse(ResponseInterface):
    ...
    def from_entity(cls, entity: Any) -> "PaymentRequestResponse":
        raise NotImplementedError(
            "PaymentRequestResponse is created directly, not from entity"
        )
    ...
    def from_entity(cls, order_item: OrderItem) -> "OrderItemResponse":
        ...
        return cls(
            ...
            additional_ingredients=[
                ing.internal_id for ing in order_item.additional_ingredient
            ],
            remove_ingredients=[
                ing.internal_id for ing in order_item.remove_ingredient
            ],
            item_receipt=[
                {
...
class OrderResponse(ResponseInterface):
    ...
    def from_entity(cls, order: Order) -> "OrderResponse":
        ...
        return cls(
            ...
            order_items=[
                OrderItemResponse.from_entity(item) for item in order.order_items
            ],
...
class OrderListResponse(ResponseInterface):
    ...
    @classmethod
    def from_entity(
        cls, orders: List[Order], skip: int = 0, limit: int = 100
    ) -> "OrderListResponse":
