...
class SSMParameterStore:
    ...
    def get_parameter(self, parameter_name: str, decrypt: bool = True) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"AWS configuration error when retrieving {parameter_name}: {e}"
            )
            raise
    ...
    def get_parameters(
        self, parameter_names: list, decrypt: bool = True
    ) -> Dict[str, str]:
        """
    ...
    def get_parameter_with_fallback(
        self, parameter_name: str, fallback_value: str, decrypt: bool = True
    ) -> str:
        """
        ...
        try:
            ...
            if value is not None:
                ...
                logger.info(
                    f"Parameter {parameter_name} not found, using fallback value"
                )
                return fallback_value

        except Exception as e:
            logger.warning(
                f"Error retrieving parameter {parameter_name}, using fallback: {e}"
            )
            return fallback_value
...
def set_aws_credentials(
    aws_access_key_id: str, aws_secret_access_key: str, aws_session_token: str = None
) -> bool:
    """
    ...
    try:
        ...
        if _ssm_client:
            _ssm_client.update_credentials(
...
def get_aws_credentials_status() -> dict:
    ...
    return {
        ...
        "credential_source": (
            "global" if all(_aws_credentials.values()) else "environment/default"
        ),
    }
