...
def set_aws_credentials_endpoint(
    ...
    try:
        ...
        if success:
            ...
            return {"status": "failed", "message": "Failed to set AWS credentials"}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error setting AWS credentials: {str(e)}"
        )
...
def get_aws_credentials_status_endpoint():
    ...
    """
    try:
        return {"status": "success", "aws_credentials": get_aws_credentials_status()}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error getting AWS credentials status: {str(e)}"
        )
...
def clear_aws_credentials_endpoint():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error clearing AWS credentials: {str(e)}"
        )
