...
def test_order_item_request_to_dict():
    req = OrderItemRequest(
        ...
        additional_ingredient_internal_ids=[2],
        remove_ingredient_internal_ids=[3],
    )
    d = req.to_dict()
...
def test_payment_request_to_dict():
    req = PaymentRequest(
        transaction_id="t", approval_status=True, date=datetime.now(), message="ok"
    )
    d = req.to_dict()
...
def test_payment_request_response_to_dict():
    resp = PaymentRequestResponse(
        order_id=1, amount=10.0, transaction_id="t", payment_url="url", expires_at="now"
    )
    d = resp.to_dict()
    assert d["order_id"] == 1
...
def test_order_item_response_to_dict():
    resp = OrderItemResponse(
        ...
        remove_ingredients=[3],
        item_receipt=[
            {"ingredient_internal_id": 2, "ingredient_name": "Queijo", "quantity": 1}
        ],
        price=10.0,
    )
    d = resp.to_dict()
