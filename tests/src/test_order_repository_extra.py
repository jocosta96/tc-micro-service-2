...
def test_order_repository_list_all():
    class Repo(OrderRepository):
        ...
        def get_by_id(self, order_internal_id): return make_order()
        def get_by_status(self, status): return [make_order()]
        def list_all(self, skip=0, limit=100): return [make_order()]
        def update(self, order): return order
        def cancel(self, order_internal_id): return True
        def update_status(self, order_internal_id, status): return make_order()
        def process_payment(self, order_internal_id, payment_data): return make_order()
